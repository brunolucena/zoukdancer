...
class Metronome extends React.Component {
  ...
  constructor(props) {
    ...
    if (this.props.subdivision < 1 || this.props.subdivision > 4) {
      throw new Error(`aaa: the \`subdivision\` prop must be between 1 and 4.`);
    }

    this.ticksPerBeat =
      this.props.beatsPerMeasure % 3 === 0 || this.props.subdivision % 3 === 0 ? TICKS_PER_BEAT_TERNARY : TICKS_PER_BEAT_BINARY;
  ...
  runScheduler = () => {
    while (this.nextNoteTime < this.audioContext.currentTime + SCHEDULE_AHEAD_TIME) {
      ...
      this.nextNoteTime += (this.state.beatsPerMeasure / this.ticksPerBeat) * secondsPerBeat;
      this.currentBeat++;
  ...
  tick = (beat, time) => {
    ...
    const isQuarterBeat = beat % (this.ticksPerBeat / this.state.beatsPerMeasure) === 0;
    const isTripletBeat =
      this.ticksPerBeat === TICKS_PER_BEAT_TERNARY && beat % (this.ticksPerBeat / this.state.beatsPerMeasure) !== 0;
    const isEighthBeat = beat % (this.ticksPerBeat / (this.state.beatsPerMeasure * 2)) === 0;
    ...
    if (this.state.subdivision === 4) {
      playTick = true;
      osc.frequency.setTargetAtTime(this.props.subdivisionFrequency, this.audioContext.currentTime, 0.001);
      gainNode.gain.setTargetAtTime(this.props.subdivisionVolume, this.audioContext.currentTime, 0.001);
    }
    ...
    if (isQuarterBeat) {
      playTick = true;
      osc.frequency.setTargetAtTime(this.props.subdivisionFrequency, this.audioContext.currentTime, 0.001);
      gainNode.gain.setTargetAtTime(this.props.beatVolume, this.audioContext.currentTime, 0.001);
    }

    if (isFirstBeat) {
      playTick = true;
      osc.frequency.setTargetAtTime(this.props.beatFrequency, this.audioContext.currentTime, 0.0001);
      gainNode.gain.setTargetAtTime(this.props.beatVolume, this.audioContext.currentTime, 0.001);
    }
    ...
    if (isFirstBeat || isQuarterBeat) {
      this.setState(
        (state) => ({
          beat: state.beat === this.state.beatsPerMeasure ? 1 : state.beat + 1 || 1,
        }),
        () => {
    ...
    if (playTick) {
      ...
      this.setState(
        (state) => ({
          subBeat: state.subBeat === this.state.subdivision ? 1 : state.subBeat + 1 || 1,
        }),
        () => {
